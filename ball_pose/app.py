...
@app.route('/pose_data/<filename>')
def get_pose_data(filename):
    """获取人体骨骼检测数据"""
    json_path = f'video/output_pose/results_{os.path.splitext(filename)[0]}.json'
...
@app.route('/debug_pose_data/<filename>')
def debug_pose_data(filename):
    """调试人体骨骼检测数据，仅返回第一帧的关键点数据"""
    json_path = f'video/output_pose/results_{os.path.splitext(filename)[0]}.json'
    ...
    try:
        ...
        # Just return first frame data for debugging
        if pose_data and 'instance_info' in pose_data and len(pose_data['instance_info']) > 0:
            first_frame = pose_data['instance_info'][0]
            return jsonify({
                "frame_id": first_frame.get("frame_id"),
                "instances_count": len(first_frame.get("instances", [])),
                "first_instance_sample": first_frame.get("instances", [])[0] if first_frame.get("instances") else None,
                "meta_info": pose_data.get("meta_info", {})
            })
        return jsonify({"error": "No frame data found"})
    except Exception as e:
        import traceback
        return f"Error: {str(e)}<br>Traceback: <pre>{traceback.format_exc()}</pre>", 500
...
def delete_file(filename):
    ...
    # 删除文件部分添加以下内容:
    pose_output_path = os.path.join('video/output_pose', filename)
    if os.path.exists(pose_output_path):
        os.remove(pose_output_path)

    pose_json_path = os.path.join('video/output_pose', f"results_{os.path.splitext(filename)[0]}.json")
    ...
    if os.path.exists(pose_frame_dir):
